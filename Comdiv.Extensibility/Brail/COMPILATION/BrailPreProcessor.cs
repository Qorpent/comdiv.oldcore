...
// COMDIV:
//   1. avoid boo 0.9.2 - ?some not allowed error on parse
using System;
...
namespace Comdiv.Extensibility.Brail {
	public class BrailPreProcessor : AbstractCompilerStep {
        ...
        private const string TripleDoubleQuote = "\"\"\"";

        private static readonly Regex escapeParametersStartingWithQuestionMarkRegEx
            = new Regex(@"(?<![<\w\d])\?([_\w][_\w\d]*)", RegexOptions.Compiled);

        private static readonly IDictionary separators = CreateSeparators();
        private readonly IBooViewEngine booViewEngine;
        private readonly IDictionary inputToCode = new Hashtable();

        private bool boomode = false;

        public BrailPreProcessor(IBooViewEngine booViewEngine, bool boomode) {
            this.booViewEngine = booViewEngine;
            this.boomode = boomode;
        }
        ...
        public override void Run() {
            ...
            foreach (ICompilerInput input in Parameters.Input) {
                using (TextReader reader = input.Open()) {
                    ...
                    if (WSAHelper.IsWSA(code)) {
                        if (null == booViewEngine || booViewEngine.ConditionalPreProcessingOnly(input.Name) == false ||
                    ...
                    code = code.replace(@"\$(?=[^{])", "_QUOTED_USD_");
        ...
        private static string EscapeNullPropagationsInOutputExpression(string code) {
            ...
            while (position < code.Length) {
                ...
                if (nextOpeningQuotePosition >= 0) {
                    ...
                    escapedCode.Append(code.Substring(nextOpeningQuotePosition,
                                                      positionAfterEndOfClosingQuote - nextOpeningQuotePosition));
        ...
        private static IList<ExpressionPosition> GetExpressionsPositions(string code) {
            ...
            for (int index = 0; index < code.Length; index++) {
                if (code[index] == '{') {
                    bracesPositions.Add(new ExpressionPosition(index - 1, -1, prevCharWasDollar || prevCharWasBang,
                                                               prevCharWasBang));
                }
            ...
            foreach (DictionaryEntry entry in separators) {
                if (code.IndexOf(entry.Key as string, 0) != -1) {
                    ...
                    if (start != null && entry.Key.ToString().IndexOf(start as string) == -1) {
                        throw new Exception("Can't mix seperators in one file. Found both " + start + " and " +
                                            entry.Key);
                    }
