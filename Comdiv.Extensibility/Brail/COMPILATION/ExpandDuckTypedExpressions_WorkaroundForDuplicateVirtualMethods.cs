    ...
    public class ExpandDuckTypedExpressions_WorkaroundForDuplicateVirtualMethods : ExpandDuckTypedExpressions{
        ...
        public static object GetProperty(object target, string name){
            try {
                var duck = target as IQuackFu;
                if (null != duck) {
                    return duck.QuackGet(name, null);
                }
                var type = target as Type;
                if (null == type) {
                ...
                throw new Exception("Error calling property "+name+" on  object "+target==null?"NULL":target.ToString(),ex);
            }
        }
        ...
        public static object GetSlice(object target, string name, object[] args){
            var duck = target as IQuackFu;
            if (null != duck){
                return duck.QuackGet(name, args);
            }
        ...
        public static object SetSlice(object target, string name, object[] args){
            var duck = target as IQuackFu;
            if (null != duck){
                return duck.QuackSet(name, (object[]) RuntimeServices.GetRange2(args, 0, args.Length - 1),
                                     args[args.Length - 1]);
            }
        ...
        private static MemberInfo SelectSliceMember(MemberInfo[] found, ref object[] args, SetOrGet sliceKind){
            ...
            object state;
            return
                Type.DefaultBinder.BindToMethod(DefaultBindingFlags | BindingFlags.OptionalParamBinding, candidates,
                                                ref args, null,
                                                null, null, out state);
        }
        ...
        private IMethod GetResolvedMethod(IType type, string name){
            ...
            if (null == method){
                throw new ArgumentException(string.Format("Method '{0}' not found in type '{1}'", type, name));
            }
            return method;
        }
