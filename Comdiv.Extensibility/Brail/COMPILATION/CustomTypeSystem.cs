...
namespace Comdiv.Extensibility.Brail{
    ...
    public class InitializeCustomTypeSystem : AbstractCompilerStep{
        public override void Run(){
			...
			Context.RegisterService(new CallableResolutionService(Context));
			Context.RegisterService(new GenericsServices());
#else
            Context.RegisterService<TypeSystemServices>(new CustomTypeSystem(Context));
