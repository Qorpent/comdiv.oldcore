...
namespace Castle.MonoRail.Views.Brail
{
	...
	public partial class BooViewEngine : ViewEngineBase,IBooViewEngine

	{
		public event Action<string> ViewRecompiled = delegate { };

		private static MonoRailViewEngineOptions options;
		...
		public MonoRailViewEngineOptions Options
		{
			get { return options; }
			set { options = value; }
		}
        ...
        public ViewStatisticsCollection Statistics { get; private set; }
        public static BooViewEngine FirstInstance { get; set; }
		...
		public void Initialize()
		{
            ...
            lock (this) {
                ...
                {
                    if (null == ViewFactory) {
                        this.ViewFactory = new MONORAILBrailTypeFactory(new BrailSourceResolver(),this.Options);
                    }
                    ...
                    if (!string.IsNullOrEmpty(options.SavePreprocessedDirectory)) {
                        basePreprocessSavePath = Path.Combine(baseDir, options.SavePreprocessedDirectory);
                        Directory.CreateDirectory(basePreprocessSavePath);
                    }
	    ...
	    public ViewTypeFactory ViewFactory { get; set; }
			...
			try
			{
				...
				BrailBase view = GetCompiledScriptInstance(file,
					//we use the script just to build the generator, not to output to the user
														   new StringWriter(),
														   context, controller, controllerContext,templateName);
			...
			if (controllerContext.LayoutNames != null && controllerContext.LayoutNames.Length != 0)
			{
				foreach (string layoutName in controllerContext.LayoutNames)
				{
					...
					BrailBase layout = GetCompiledScriptInstance(layoutFilename, output,
													   context, controller, controllerContext,layoutTemplate);
		...
		public BrailBase GetCompiledScriptInstance(
			...
			IController controller, IControllerContext controllerContext, string  templatename)
		{
			lock(this) {
				if (!initialized) {
					Initialize();
				...
				if (UseFactory) {
					var view = this.ViewFactory.CreateView<BrailBase>(templatename,
					                                                  new[]
						                                                  {
							                                                  typeof (BooViewEngine), typeof (TextWriter),
							                                                  typeof (IEngineContext), typeof (IController),
							                                                  typeof (IControllerContext)
						                                                  },
					                                                  new object[]
						                                                  {
							                                                  this, output, context, controller,
							                                                  controllerContext
						                                                  });
					...
					type = CompileScript(templatename, filename, batch);
					...
					if (options.CollectStatistics) {
						sw.Stop();
						Statistics.Compiled(templatename, sw.ElapsedTicks);
					}
		...
		public Type CompileScript(string viewname, string filename, bool batch)
		{
            if (!initialized) Initialize();
			...
			CompilationResult result = DoCompile(inputs2FileName.Keys, name,viewname);
			...
			if (result.Context.Errors.Count > 0)
			{
				...
				return CompileScript(viewname,filename, false);
			}
			Type type;
			foreach (ICompilerInput input in inputs2FileName.Keys)
			{
				string viewName = Path.GetFileNameWithoutExtension(input.Name);
				string typeName = MONORAILTransformToBrailStep.GetViewTypeName(viewName);
			...
			type = (Type)compilations[filename];
                ...
                var keys = _outputCache.Keys.Where(x => x.StartsWith(type.FullName + "_")).ToArray();
		...
		public ICompilerInput CreateInput(string name)
		{
			...
			{
                //if preprocessors not configured - process usual way
                if (0 == Options.Preprocessors.Count){
                    return new StringInput(name, stream.ReadToEnd());
                }
                //else we primary preprocess file content
                else{
                    string srcContent = preprocess(name,stream);
                    return new StringInput(name, srcContent);
                }
			}
		}

	    private string preprocess(string name,StreamReader stream) {
	        var srcContent = stream.ReadToEnd();
	        foreach (var preprocessor in Options.Preprocessors.OrderBy(p=>p.Idx)){
	            srcContent = preprocessor.Preprocess(srcContent);
	        }
            if(!string.IsNullOrEmpty(basePreprocessSavePath)){
                
                var saveFileName = Path.Combine(basePreprocessSavePath, name.Replace("/", "_").Replace("\\", "_"));
	    ...
	    private static BooCompiler SetupCompiler(IEnumerable<ICompilerInput> files,bool boomode)
		{
			...
			{
                if(assembly.GetName().Name.EndsWith("Model.Implementation")) {
                    continue;
                }
