		...
		from : function(url/*or controller name*/, action /*if given first treats as controller*/, extension /* rails by default MonoRail */){
			...
			var result = {
				...
				_loadCSS : function(){
					...
					if(!($(elementid))){
						document.body.insert({bottom: new Element("style",{type:"text/css", id:elementid})});
				...
				eval : function(param){
					...
					if(script){
						...
						//if param exists, treats eval result as function and returns it's result
						if(typeof(result)==typeof(function(){})){
							return result(param || {});
						}else{
							return result;
						}
